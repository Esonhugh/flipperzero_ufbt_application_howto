...
static void animateSet(animID_t, state_t* const, Gui*);
...
//
static void animateSet(animID_t id, state_t* const state, Gui* gui) {
    ENTER;
    furi_assert(state);
    furi_assert(gui);
    ...
    case ANIM_TEXT:
        ...
        state->minX = 1;
        state->maxX = (state->cnvW - 1) - canvas_string_width(canvas, state->text);
        state->minY = 1;
        state->maxY = (state->cnvH - 1) - canvas_get_font_params(canvas, state->font)->height;
        break;
...
//
static inline bool stateInit(state_t* const state, Gui* gui) {
    ENTER;
    furi_assert(state);
    furi_assert(gui);
